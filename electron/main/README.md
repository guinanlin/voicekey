...
- `voice-command-runner.ts` - PTT 转写与 HTTP `/clipboard/type` 识别末尾触发词（含 `[小猪佩奇:微信]` 方括号标记），按 `voiceCommands` 调用 `textLlm` 生成再注入。
...
- `history-manager.ts` - 录音历史存储（最多 1000 条），提供增删清空、统计，以及按历史记录 + 指令保存/恢复工匠会话。
...
- `http-server.ts` - Fastify HTTP 服务器，暴露剪贴板/输入 REST API（端口 4321），自动生成 OpenAPI/Swagger 文档；`/clipboard/type` 支持末尾智能指令（`[小猪佩奇:微信]` 等），成功后写入 `history-manager`（剥离指令的正文 + 关联工匠会话）并向所有窗口广播 `history:changed`。
