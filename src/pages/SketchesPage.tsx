...
export default function SketchesPage() {
  ...
  const onSessionSummarize = useCallback(async () => {
    ...
    if (!cfg.textLlm?.apiKey?.trim()) {
      toast.error(t('sketches.summaryError.no_api_key'))
      return
    }

    setSummarizing(true)
    try {
      const systemPrompt = t('sketches.summaryPromptDefault')
      const result = await api.generateFlashSummary(selectedSession.sessionId, systemPrompt)
      if (result.ok) {
        toast.success(t('sketches.summarySuccessToast'))
        await loadFlashData()
        setRightTab('summary')
      } else {
        const { code } = result
        if (code === 'request_failed' && result.message) {
          toast.error(t('sketches.summaryError.request_failed', { message: result.message }))
        } else {
          toast.error(t(`sketches.summaryError.${code}`))
        }
      }
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t('common.unknownError'))
  ...
  }, [activeSession, loadFlashData, selectedSession, summarizing, t])
            ...
            <>
              <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b px-3 py-2.5">
                <Button
                  ...
                  disabled={!!activeSession || summarizing}
                  onClick={() => {
                    void onSessionSummarize()
                  }}
                >
                  <Sparkles className="size-4 shrink-0" aria-hidden />
                  {summarizing ? t('sketches.summaryGenerating') : t('sketches.summaryAiButton')}
                </Button>
                <Dialog>
