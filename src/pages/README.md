...
- `HistoryPage.tsx` - 工匠页（路由 `/history`）：工具栏条数右侧小图标可折叠/展开右侧 `ChatPanel`（默认收起）；左侧历史条目悬停指令按钮（润色/总结等）→ 展开右栏并以对应 `voiceCommands` Prompt 自动发起聊天；展开后可拖动分隔条调比例；状态持久化 `localStorage`。
